...
export const buildResolvedGraphModel = (
  ...
  for (const key of graph.nodeOrder) {
    ...
    const nodeStyleKind = node.resolutionError
      ? "Error"
      : def
        ? getNodeType(def)
        : "Error";
    ...
    nodes.push({
      ...
      inputs: (node.input ?? []).map((input) => ({ label: input, variable: input || undefined })),
...
export const computeSearchState = (
  ...
  for (const key of graph.nodeOrder) {
    ...
    if (current.mode === "id") {
      ...
      matched = stringParts.some((part) => includesString(part, current.query, current.caseSensitive));
    }
    if (matched) {
      resultKeys.push(key);
    }
  ...
  return {
    ...
    resultKeys,
    activeResultIndex:
      resultKeys.length === 0 ? 0 : Math.min(current.activeResultIndex, resultKeys.length - 1),
  };
};
