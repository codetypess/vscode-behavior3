      ...
      <div className="b3-v2-inspector-content">
        ...
        >
          ...
          <Form.Item label="Description" name="desc">
            <TextArea autoSize={{ minRows: 2, maxRows: 5 }} onBlur={() => void form.submit()} />
          </Form.Item>
            ...
            <>
              ...
              <Form.Item label="Group" name="group">
                <Select
                  ...
                  options={groupDefs.map((group) => ({ label: group, value: group }))}
                ...
                {fields.map((field) => (
                    ...
                    rules={[
                      {
                        validator: async (_, value: VariableRowValue) => {
                          ...
                          if (!value.desc?.trim()) {
                            throw new Error("Variable description is required");
                          }
                        },
                      },
                    ]}
                  >
                    <VariableDeclRow
                      ...
                      onFocusVariable={(name) => void runtime.controller.focusVariable([name])}
                    />
                  </Form.Item>
                ))}
                <Form.Item style={{ marginBottom: 0, marginTop: 4 }}>
                  <Button type="dashed" block icon={<PlusOutlined />} onClick={() => add({ name: "", desc: "" })}>
                    Add Variable
                  </Button>
            ...
            {(fields, { add, remove }, { errors }) => (
              <div className="b3-v2-list-block">
                {fields.map((field) => {
                  ...
                  const importVars = currentPath ? importDeclByPath.get(currentPath) ?? [] : [];

                  return (
                    <div key={field.key} className="b3-v2-decl-group">
                      <Flex gap={4} align="start">
                        <Form.Item label="Path" name={[field.name, "path"]} style={{ flex: 1, marginBottom: 2 }}>
                          <AutoComplete
                ...
                <Form.Item style={{ marginBottom: 0, marginTop: 4 }}>
                  <Button type="dashed" block icon={<PlusOutlined />} onClick={() => add({ path: "" })}>
                    Add Import
                  </Button>
                  <Form.ErrorList errors={errors} />
                </Form.Item>
