...
import { useDocumentStore, useRuntime, useSelectionStore, useWorkspaceStore } from "../../app/runtime";
import {
  OverrideBar,
  SectionDivider,
  cleanSlotLabel,
...
const NodeArgField: React.FC<{
  ...
  if (hasArgOptions(arg)) {
    return (
      <Form.Item label={arg.desc || arg.name} name={["args", arg.name]} rules={[{ validator: validateField }]}>
        <Select
  ...
  if (isStringType(type)) {
    return (
      <Form.Item label={arg.desc || arg.name} name={["args", arg.name]} rules={[{ validator: validateField }]}>
        <TextArea autoSize={{ minRows: 2, maxRows: 5 }} disabled={disabled} onBlur={onCommit} />
      </Form.Item>
    );
  }
...
export const NodeInspectorForm: React.FC = () => {
  ...
  const effectiveName = (watchedName ?? selectedNode?.data.name ?? "").trim() || selectedNode?.data.name || "";
  ...
  const isInputOverridden = (index: number, variadic = false) => {
    ...
    if (variadic) {
      return !compareJsonValue(selectedNode.data.input?.slice(index) ?? [], subtreeOriginal?.input?.slice(index) ?? []);
    }
  ...
  const isOutputOverridden = (index: number, variadic = false) => {
    ...
    if (variadic) {
      return !compareJsonValue(selectedNode.data.output?.slice(index) ?? [], subtreeOriginal?.output?.slice(index) ?? []);
    }
    return (selectedNode.data.output?.[index] ?? "") !== (subtreeOriginal?.output?.[index] ?? "");
  };
  ...
  const isArgOverridden = (argName: string) => {
    ...
    return !compareJsonValue(selectedNode.data.args?.[argName], subtreeOriginal?.args?.[argName]);
  };
  ...
  const buildInputArray = (currentNodeDef: NodeDef | null, values: Record<string, unknown>) => {
    ...
    currentNodeDef.input.forEach((slot, index) => {
      ...
      if (slot === currentNodeDef.input![currentNodeDef.input!.length - 1] && isVariadic(currentNodeDef.input!, index)) {
        return;
      }
    });
  ...
  const resetInputField = (index: number, variadic = false) => {
    form.setFieldValue(
      ["inputSlots", index],
      variadic ? subtreeOriginal?.input?.slice(index) ?? [] : subtreeOriginal?.input?.[index] ?? ""
    );
    queueSubmit(form);
  };
  ...
  const resetOutputField = (index: number, variadic = false) => {
    form.setFieldValue(
      ["outputSlots", index],
      variadic ? subtreeOriginal?.output?.slice(index) ?? [] : subtreeOriginal?.output?.[index] ?? ""
    );
    queueSubmit(form);
  };

  const resetArgField = (arg: NodeArg) => {
    form.setFieldValue(["args", arg.name], formatArgInitialValue(arg, subtreeOriginal?.args?.[arg.name]));
    queueSubmit(form);
  };
    ...
    <>
      ...
      <div className="b3-v2-inspector-content">
        <Form
          ...
          onFinish={(values) => {
            try {
              const currentNodeDef = nodeDefs.find((entry) => entry.name === String(values.name ?? "").trim()) ?? null;
              ...
              void runtime.controller.updateNode({
                ...
                data: {
                  name: String(values.name ?? selectedNode.data.name).trim() || selectedNode.data.name,
        ...
        >
          ...
          {nodeDef?.group?.length ? (
            <Form.Item label="Group" name="group">
              <Select
                mode="multiple"
                disabled
                options={nodeDef.group.map((group) => ({ label: group, value: group }))}
              />
            </Form.Item>
          ) : null}
            ...
            rules={[
              {
                validator: async (_, value) => {
                  ...
                  if (!nodeDefMap.has(nextName)) {
                    throw new Error(`Unknown node: ${nextName || selectedNode.data.name}`);
                  }
                },
              },
            ]}
          ...
          >
            <Form.Item label="Description" name="desc">
              <TextArea autoSize={{ minRows: 2, maxRows: 5 }} disabled={fieldEditDisabled} onBlur={submitNodeForm} />
            </Form.Item>
          </OverrideBar>

          <OverrideBar
            active={canShowOverride && Boolean(selectedNode.data.debug) !== Boolean(subtreeOriginal?.debug)}
            onReset={() => {
          ...
          >
            <Form.Item label="Debug" name="debug" valuePropName="checked">
              <Switch disabled={fieldEditDisabled && !selectedNode.data.path} onChange={() => queueSubmit(form)} />
            </Form.Item>
          </OverrideBar>

          <OverrideBar
            active={canShowOverride && Boolean(selectedNode.data.disabled) !== Boolean(subtreeOriginal?.disabled)}
            onReset={() => {
          ...
          {nodeDef?.doc ? <ReactMarkdown className="b3-v2-markdown">{nodeDef.doc}</ReactMarkdown> : null}
                                    ...
                                    rules={[
                                      {
                                        validator: async (_, value: string | undefined) => {
                                          ...
                                          if (
                                            ...
                                            !checkOneof(
                                              relatedArg,
                                              form.getFieldValue(["args", relatedArg.name]),
                                              value
                                            )
                                          ) {
                                            throw new Error(
                      ...
                      rules={[
                        ...
                        },
                        {
                          validator: async (_, value: string | undefined) => {
                            ...
                            if (relatedArg && !checkOneof(relatedArg, form.getFieldValue(["args", relatedArg.name]), value)) {
                              throw new Error(`Only one of '${relatedArg.name}' and '${slotLabel}' can be set`);
                            }
                          },
                        },
                      ]}
