...
export const createVariableOptions = (
  ...
  dfs(root, (node) => {
    ...
    node.input?.forEach((variable, index) => {
      ...
      const rawLabel =
        nodeDef?.input?.length && index >= nodeDef.input.length && isVariadic(nodeDef.input, -1)
    ...
    node.output?.forEach((variable, index) => {
      ...
      const rawLabel =
        nodeDef?.output?.length && index >= nodeDef.output.length && isVariadic(nodeDef.output, -1)
