    ...
    connect(onMessage) {
      const handler = (event: MessageEvent<HostToEditorMessage>) => {
        ...
        if (message.type === "varDeclLoaded") {
          onMessage({ type: "varDeclLoaded", payload: normalizeHostVarsMessage(message) });
          return;
        }
        ...
        if (message.type === "settingLoaded") {
          onMessage({ type: "settingLoaded", nodeDefs: message.nodeDefs });
          return;
        }

        if (message.type === "buildResult") {
          onMessage({ type: "buildResult", success: message.success, message: message.message });
        }
      };
