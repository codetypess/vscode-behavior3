...
export const onSetup = (env) => {
  ...
  const selfPath = env.path.join(env.workdir, "scripts", "build.js");
  console.log(env.path.dirname(selfPath));
  // Always resolve paths from env.workdir — extension host cwd is not the project root.
  console.log(env.fs.readFileSync(selfPath, "utf8"));
};
