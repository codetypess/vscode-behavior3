...
import {
    ...
    getCachedInspectorNodeSnapshot,
...
export const editorStateSharedTests = defineSharedTests([
    ...
    {
        name: "stores the current selected node snapshot for same-node pending inspector recovery",
        run() {
            const filePath = "/tmp/embedded-focus-regression.json";
            const selectedNodeRef: NodeInstanceRef = {
                instanceKey: "5",
                displayId: "5",
                structuralStableId: "child",
                sourceStableId: "child",
            ...
            const reboundRef: NodeInstanceRef = {
                ...selectedNodeRef,
                instanceKey: "7",
                displayId: "7",
            };
            ...
            rememberInspectorNodeSnapshotFromSelection(filePath, {
                selectedNodeRef,
                selectedNodeSnapshot,
            });

            assert.deepEqual(
                getCachedInspectorNodeSnapshot(filePath, reboundRef),
                cloneInspectorNodeSnapshotForRef(selectedNodeSnapshot, reboundRef)
            );
        },
    },
    {
