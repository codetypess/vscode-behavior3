...
- Inspector：按配置在独立侧栏或主编辑器内编辑树级与节点级字段
...
2. Inspector 支持 `sidebar` 与 `embedded` 二选一：
   - `sidebar`：VS Code `WebviewViewProvider` 承载独立 Inspector Sidebar
   - `embedded`：主编辑器 webview 内嵌 Inspector 面板
...
- 在 Inspector 中编辑树元数据、变量、import 引用、节点字段、输入输出槽与结构化参数
- 在主编辑器与 Inspector 展示面之间同步选中节点、当前文档内容和变量视图
...
### 3. 主编辑器与 Inspector 展示面共享同一业务语义

Inspector 不是另一套业务实现，而是复用同一套协议、字段和命令语义。`sidebar` 模式下通过宿主代理消息落到当前激活编辑器；`embedded` 模式下则直接运行在当前 editor webview 内。
...
- 根据当前激活的 Behavior3 编辑器或当前 editor webview 同步上下文
- Tree Inspector 与 Node Inspector 共用当前文档状态
- `sidebar` 模式下支持代理树/节点变更、保存、撤销、重做
- `embedded` 模式下在主编辑器内部直接编辑
- 在无激活编辑器或非当前模式入口时显示空状态或提示态
...
1. 打开树文件后，主编辑器和当前启用的 Inspector 展示面都能得到一致的文档与选中上下文。
