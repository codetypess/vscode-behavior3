...
Public declaration files import those model types from `b3type.ts`. The npm package file list includes `b3type.ts`, which now directly owns `VERSION`, so `build.d.ts` remains type-resolvable without `b3model.d.ts`.
...
4. Add `b3type.ts` to the package file list.
...
Mitigation: include `b3type.ts` in `package.json` files and verify with `npm run pack:npm`.
