...
- Node Inspector 展示节点 `displayId`、`uuid`、类型、children、name、desc、debug、disabled、path
- 已知 nodeDef 默认走结构化字段编辑；在 `sidebar` 模式下可通过顶部 `JSON` 按钮切换到只读原始 JSON 视图
- 未知节点默认显示只读原始 JSON 视图
...
- 在 `unknown` 节点中临时把 `name` 改成已知节点名称时，参数预览区也不会显示上一节点残留的参数值
- 在上述场景失焦后，旧参数值也不会因为嵌套表单 merge 而重新出现并写回当前节点
- 对 JSON 中未显式保存参数、但 nodeDef 定义了默认参数的主树节点，图节点与 Inspector 参数区都显示当前默认值；只读 JSON 仍保持未写入该 `args`
- 对上述默认参数字段，单纯聚焦/失焦不会把默认值写回文件；显式编辑后才会提交当前值
- 对上述默认参数字段，右侧显示独立重置按钮；点击后先弹出确认，再清除当前显式值并恢复为“使用默认值”的状态
