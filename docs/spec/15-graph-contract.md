...
- `icon`
- `nodeStyleKind`
- `nodeStyleKind` 的 `Error` 包括 resolution error、共享 validation diagnostic（含固定 children 数量不匹配、required / oneof 等节点约束）和 custom checker diagnostic
- `accentColor`
- `debug`
- `disabled`
- `hasOverride`
...
- G6 负责树布局
- controller 不参与坐标计算
- 节点备注、参数、输入、输出等元信息文本必须使用图节点内部共享的内容宽度、行高与纵向步进测量和绘制；多行文本优先在自然断点换行，wrapped continuation 与 section transition 使用同一 vertical rhythm，且高亮背景高度应覆盖全部文本行
